...
\begin{lemma}
    For any admissible k-tuple $A$, $\Omega_A(s)$ converges for any $s$ with $\Re(s)>1/2$.
\end{lemma}
\begin{proof}
    Let $A$ be an admissible k-tuple.
    Fix $s=\sigma + it$ with $\sigma>1/2$.
    Let 
    $$\Omega_{A,r}(s) = \prod_{p>r}\frac{(1-p^{-s})^{\#A}}{1-\#Ap^{-s}}.$$
    We will show the convergence of $\Omega_{A,r}$ at $s$ for a fixed $r$, from which 
    the result follows. 
    We fix $r$ to be large enough that $\#Ar^{-\sigma} < 1/2$.
    We have
    \begin{align*}
        \log{\Omega_{A, r}}(s) 
...
\begin{proof}
    ...
    \begin{align*} 
        \frac{1}{C_A} 
        &= \prod_{p}\frac{(1-p^{-1})^{\#A}}{1-u_A(p)p^{-1}} \\
        &= \prod_{p\not\in S_A}\frac{(1-p^{-1})^{\#A}}{1-\#Ap^{-1}}
           \prod_{p\in S_A}\frac{(1-p^{-1})^{\#A}}{1-u_A(p)p^{-1}} \\
        &= \prod_{p}\frac{(1-p^{-1})^{\#A}}{1-\#Ap^{-1}}
           \prod_{p\in S_A}\frac{1-\#Ap^{-1}}{1-u_A(p)p^{-1}} \\
        &= \Omega_A(1)N_A(1).
    \end{align*}

    Taking residues in (\ref{DuA normal form}) yields the second equality.
\end{proof}
...
\begin{enumerate}
  \item
    This one website gets prime constant in terms of prime zeta, what
    do we get following that argument for u or w series?
  \item
  ...
  \item
    ...
    - No, 0,30 and 0,2,6 counterexample
    But maybe among same size?
    \begin{conjecture}
        Let $A,B$ be admissible k-tuples with $\#A=\#B$.
        Then $$M_A(1) < M_B(1) \iff N_A(1) < N_B(1).$$
    \end{conjecture}
  \item
