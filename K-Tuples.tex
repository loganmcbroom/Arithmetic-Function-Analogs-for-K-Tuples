...
case. 
We denote the wrapped with $\varrho$ rather than $\varUpsilon$,
...
\subsection{Totients as Convolutions}

We may also obtain many totient-like functions in terms of
discrete modular convolutions.
\begin{definition}
    Given $f,g : \mathbb{Z}_n \rightarrow \mathbb{C}$,
    define the discrete modular convolution $\odot_n$ by
    $$(f\odot g)(k) = \sum_{\substack{a+b\equiv k \\ \bmod n}}f(a)g(b),$$
...
We get the totients by asking questions about the convolution.
For $\varphi_A$, the question is "how many times did we get to $\#A_n$?".
Everything else is a question about how many times different values
were obtained.
The questions are so similar to the original definitions,
it seems like the convolution is just a way to describe
the queried set as a function.
That is, instead of asking for the number of $k$ for which
$q = \#S$, we are asking for the number of $k$ for which
a convolution takes on certain value $q$ times.
...
\subsection{The Dirichlet Series of $w_A$}
...
\begin{enumerate}
    \item
        Maybe we can get a meaning for $u$ out of the convolution
        form for $\varphi_A$?
        As in, maybe on composite $n$, $u$ indicates some
        obvious information about the conv sequence.

    \item
        The fundamental result we require is a relationship,
        asymptotic or otherwise, between $u$ or $w$ and the
        standard $\Psi_A$.
        Such a result would likely lead to proof of HLC.
