...
is also known, at least in some well studied cases.
...
\begin{definition}
    ...
    \end{enumerate}
    Then we say $v$ is \textbf{unit looking}.
...
This condition gives a simple bound on $u_A$.
...
\section{The Dirichlet Series of $u_A$}
...
We may factor $\Di(u_A)$ as
\begin{equation}\label{DUA zeta form}
\Di(u_A)(s) = \zeta^{\#A}(s)  
    \prod_{p}\frac{(1-p^{-s})^{\#A}}{1-u_A(p)p^{-s}}.
\end{equation}
...
where the $p$ column gives those primes over which $u_A(p)\not=\#A$.
...
they should not be used outside this context.
...
but this diverges.
We can use $N_A(1)$ for any particular $A$, but opt to instead choose
the maximum of such constants.
\begin{definition}
...
\begin{proof}
    ...
    For proof, assume for contradiction there is 
\end{proof}
