...
For $\Re(s)>1$, define
$$\zeta_A(s) := \Di(w_A)(s) = \sum_{n\ge 1} \frac{w_A(n)}{n^s}.$$
\end{definition}
...
$$\zeta_A(s) = \zeta(s)\prod_{p\in S_A}\frac{1-p^{-s}}{1-w_A(p)p^{-s}}.$$
...
\begin{equation}\label{res 1}
    \text{Res}(\zeta_A, 1) = M_A(1) = \prod_{p\in S_A}\frac{p-1}{p-w_A(p)}.
\end{equation}
...
Just as we describe $\zeta_A$ in terms of $\zeta$ and a finite modulator, 
we may describe $\Di(u_A)$ as
$$\Di(u_A)(s) = \Di((\#A)^\Omega)(s)
    \prod_{p\in S_A}\frac{1-\#Ap^{-s}}{1-u_A(p)p^{-s}}.$$
We then multiply and divide by $\zeta^{\#A}$ and obtain
$$\Di(u_A)(s) = \zeta^{\#A}(s) 
    \prod_{p}\frac{(1-p^{-s})^{\#A}}{1-\#Ap^{-s}}
    \prod_{p\in S_A}\frac{1-\#Ap^{-s}}{1-u_A(p)p^{-s}}.$$
This final step is taken because the central product now has reasonable 
convergence properties.
We refer to this central product as $\Omega_A(s)$, and the finite product
once again as the modulator, this time denoted $N_A(s)$, so that
\begin{equation}\label{DuA normal form}
    \Di(u_A)(s) = \zeta^{\#A}(s) \Omega_A(s) N_A(s)
\end{equation}

\begin{lemma}
    For any admissible k-tuple $A$, $\Omega_A(s)$ converges for $s$ with $\Re(s)>1/2$.
\end{lemma}
\begin{proof}
    Let $A$ be an admissible k-tuple.
    Fix $s=\sigma + it$ with $\sigma>1/2$.
    Let 
    $$\Omega_{A,r} = \prod_{p>r}\frac{(1-p^{-s})^{\#A}}{1-\#Ap^{-s}}.$$
    We will show the convergence of $\Omega_{A,r}$ at $s$ for $r=todo$, from which 
    the result follows. 
    We have
    \begin{align*}
        \log{\Omega_{A, r}}(s) 
    ...
    \end{align*}
    For each $p>r$ the $k$ sum is a convergent geometric series.
    ...
    Thus, $\log{\Omega_{A,r}}$ converges absolutely, and exponentiating gives
    the result.
\end{proof}

We do not need the full domain of convergence of the prior result; our
goal was to justify evaluating $\Omega_A(1)$ and obtaining the
following.
...
\begin{corollary}
    ...
    $$C_A = \Omega_A(1) N_A(1),$$
    and
    $$\Res(u_A,1) = C_A\Res(\zeta^{\#A},1).$$
\end{corollary}

The Laurent expansion of $\zeta$ is known, so the residues above
are computable.
The constants $\Omega_A$ are determined entirely by $\#A$, and thus
...
The following table gives these values for some 
admissible, non-residually-equivalent k-tuples,
where the $p$ column gives those primes over which the product ranges.
The second data row gives these values for any odd prime $p$.
\begin{center}
\begin{tabular}{ c | c | c | c }
 $A$ & $p$ & $M_A(1)$ & $N_A(1)$ \\ 
 \hline
 $\{0,2\}$      & 2 & 2/3 & 1/2 \\  
 $\{0,2p\}$     & 2,$p$ & $\frac{4(p-1)}{3(2p-1)}$ & $\left(\frac{p-1}{2p}\right)^2$ \\  
 $\{0,30\}$     & 2,3,5 & 64/135 & 4/15 \\  
 $\{0,2,6\}$    & 2,3 & 18/35 & 2/9 \\  
 $\{0,4,10\}$   & 2,3,5 & $2^33^3/5\cdot7\cdot13$ & $2^7/3^35^2$ \\  
 $\{0,4,28\}$   & 2,3,7 & $2^23^4/5\cdot7\cdot19$ & $2^43/5\cdot7^2$ \\  
 $\{0,2,6,8\}$  & 2,3,5 & ? & ? \\  
 $\{0,2,6,30\}$ & 2,3,5,7 & ? & ? \\  
\end{tabular}
\end{center}
...
\begin{enumerate}
  \item
    Write program to compute N and M for large data set of k-tuples.
    We could try to order them by N or M values; do they produce the same order?
  \item
    Write out the proof for psiA in terms of psi,
    not sure if it's needed but it's cool.
  \item
    Rewrite the stupid deleted proof of the uA L series converging for
    real over one half.
  \item
    We need to understand where the HLC constants are coming from.
    The denominator seems to be accounting for the Omega growth,
    is there a way to express it in terms of $w$?
    Yes, we have $C_A = (\zeta^{\#A} / L(s,u_A))(s)$, and the
    L-series for $u_A$ and $v_A$ are related.
    $$L(s,v_A) = \zeta(s)M_A(s),$$
    $$L(s,u_A) = \zeta^{\#A}(s)M'_A(s)L(s,(\#A)^\Omega),$$
    where $M'$ 
  \item
