...
TODO
This whole derivation is a bit above my pay grade, so not exactly
up to par. 
I think it's got a cool idea though.
\begin{definition}
    $$\Psi_A(x) = \sum_{n \le x} \Lambda(n)w_A(n).$$
\end{definition}
\begin{theorem}
    For any admissible k-tuple $A$, we have
    $$\Psi^*_A(x) = \Psi^*(x) - \#S_A\log x + O(1),$$
    where the stars indicate the summation should be multiplied by 1/2
    when $x\in\mathbb{Z}$.
\end{theorem}
\begin{proof}
    We apply Perron's formula and obtain
    \begin{align*}
    ...
    \begin{align*}
    \Di(\Lambda\cdot w_A) &= -\frac{\Di'(w_A)}{\Di(w_A)} \\
    &= -\frac{\zeta' \cdot M_A + \zeta\cdot M'_A}{\zeta\cdot N_A} \\ 
    &= -\frac{\zeta'}{\zeta} -\frac{M'_A}{N_A} \\
    &= \Di(\Lambda) -\frac{M'_A}{M_A}.
    \end{align*}

    Substituting this into our integral, we then have
    \begin{align*}
        \Psi^*_A(x) &= \sum_{n\le x}^*\Lambda(n)w_A(n) \\
        &= \frac{1}{2\pi i}\int_{c-i\infty}^{c+i\infty}\left[\Di(\Lambda)(z)-\frac{M'_A}{M_A}(z)\right]
            \frac{x^z}{z}dz \\
        &= \Psi^*(x)
